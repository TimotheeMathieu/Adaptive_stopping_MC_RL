...
\title{Adaptive stopping in sequential tests to compare Deep RL Agents and improve reproducibility}
\date{}
\begin{document}
\maketitle
...
Compute the boundary $\widehat{b}_{N,k}$ as follows:
...
$$\widehat{b}_{N,k}^{(\textbf{C}^+)} = \inf \left\{b >0 :\frac{1}{((2N)!)^k}\sum_{\sigma \in S_k} \1\{T_{N,k}^{(\textbf{C}^+)}(\sigma_1^k) \ge  b\} \le  \lambda^+\right\} $$
and
$$ \widehat{b}_{N,k}^{(\textbf{C}^-)} = \sup\left\{ b>0 : \frac{1}{((2N)!)^k}\sum_{\sigma \in S_k} \1\{T_{N,k}^{(\textbf{C}^-)}(\sigma_1^k) \leq b\} \le  \lambda^- \right\} .$$
