...
\SetAlgoLined
\SetKwInput{KwParameter}{Parameters}
\KwParameter{Agents $A_1,A_2,\dots, A_m$, environment $\mathcal{E}$, comparison pairs $(c_i)_{i \le J}$ where $c_i$ is a couple of two agents that we want to compare. Integers $K,n \in \N^*$, test parameters $\alpha,\beta$.}
Define $mnK$ different seeds $(s_{i,j})_{i\le m,j\le n}$.\\
Set $\textbf{C}=\{1, \dots,J\}$ the set of indices for the comparisons we want to do.\\
\For{$k=1,\dots,K$}{
\For{$i=1...m$}{
Train agent $A_i$ on environment $\mathcal{E}$ with the seeds $s_{i,kn},\dots,s_{i,(k+1)n}$.\\
Collect evaluations $e_{kn}(A_i),\dots,e_{(k+1)n}(A_i)$.\\
}
Set $\lambda^+ = \alpha \left(\frac{k}{K}\right)- \alpha \left( \frac{k-1}{K}\right)$ and $\lambda^- = \beta \left(\frac{k}{K}\right)- \beta \left( \frac{k-1}{K}\right)$.\\
\While{}{
