...
In the case where only two agents are compared, we use the following algorithm (see Section~\ref{sec:multi} for the multi-agent algorithm).
...
Collect evaluations $e_{kn}(A_i),\dots,e_{(k+1)n}(A_i)$ using this trained agent.\\
}
Compute the boundary $B_k$ using all current and previous data.\\
\If{$|\frac{1}{nk}\sum_{j=1}^{nk}e_{j}(A_1)-\frac{1}{nk}\sum_{j=1}^{nk}e_{j}(A_2)| \ge B_k$}{
...
\paragraph{Speeding up permutation tests}
...
is the mean difference on block $i$ after permutation by $\sigma_i$. The algorithm is given in Algorithm~\ref{algo:main}
\todoT{Redo the algorithm for better rendering and more concise writing}
\begin{algorithm}[h]
\SetAlgoLined
\SetKwInput{KwParameter}{Parameters}
\KwParameter{Agents $A_1,A_2,\dots, A_m$, environment $\mathcal{E}$, comparison pairs $C=(c_1,\dots, c_J)$ where $c_i$ is a couple of two agents that we want to compare. Number of blocks $K \in \N^*$, size of a block $n$, test parameters $\alpha,\beta$, .}
Define $mnK$ different seeds $(s_{i,j})_{i\le m,j\le n}$.\\
Define $\textbf{C}=\{1, \dots,J\}$ the set of comparisons we want to do.\\
\For{$k=1,\dots,K$}{
\For{$i=1...m$}{
Train agent $A_i$ on environment $\mathcal{E}$ with the seeds $s_{i,kn},\dots,s_{i,(k+1)n}$.\\
Collect evaluations $e_{kn}(A_i),\dots,e_{(k+1)n}(A_i)$.\\
}
Set $\lambda^+ = \alpha \left(\frac{k}{K}\right)- \alpha \left( \frac{k-1}{K}\right)$.\\
Set $\lambda^- = \beta \left(\frac{k}{K}\right)- \beta \left( \frac{k-1}{K}\right)$.\\
Compute $\widehat{c}_{n,k}^{(\textbf{C}^+)}$ and $\widehat{c}_{n,k}^{(\textbf{C}^-)}$ the quantiles of order $1-\lambda^+$ and $\lambda^-$ respectively of the permutation law of $T_{n,k}^{(\textbf{C}^+)}(\sigma)$ and $T_{n,k}^{(\textbf{C}^-)}(\sigma)$ respectively:
$$\sum_{\sigma \in S_n} \1\{T_{n,k}^{(\textbf{C}^+)}(\sigma) \ge  \widehat{c}_{n,k}^{(\textbf{C}^+)}\} = \lfloor \lambda^+ n! \rfloor .$$
$$\sum_{\sigma \in S_n} \1\{T_{n,k}^{(\textbf{C}^-)}(\sigma) \leq   \widehat{c}_{n,k}^{(\textbf{C}^-)}\} = \lfloor \lambda^- n! \rfloor .$$

\If{$T_{n,k}^{(\textbf{C}^+)}(\mathrm{id}) > \widehat{c}_{n,k}^{(\textbf{C}^+)}$}{
Reject $H_{j_{\max}}$ where $j_{\max} =  \arg\max\left(T_{n,k}^{(j)}(\sigma),\quad j \in \textbf{C}\right)$.\\
Update $\textbf{C}=\textbf{C} \setminus \{j_{max}\}$
}
\If{$T_{n,k}^{(\textbf{C})}(\mathrm{id}) < \widehat{c}_{n,k}^{(\textbf{C}^-)}$}{
Accept $H_{j_{\min}}$ where $j_{\min} =  \arg\min\left(T_{n,k}^{(j)}(\sigma),\quad j \in \textbf{C}\right)$.\\
Update $\textbf{C}=\textbf{C} \setminus \{j_{max}\}$
}
\If{\textbf{C} changed}
{ Go to line 8.}
...
First we prove that the multiple testing scheme is correct for testing $H_{j,k}: P_j = P_k$ against $\overline{H}_{j,k}: P_j \neq P_k$ for all the couples $j \neq k$.
\todoT{Maybe prove that asymptotically, the multiple test is ok for stronger hypotheses.}
\begin{Theorem}
For any power-spending function $\beta$ and level-spending function $\alpha$, the test resulting from Algorithm~\ref{algo:main} has a strong control on the FWE: we have $\mathrm{FWE}\le\alpha(1)$.
\end{Theorem}
\begin{proof}
...
$$\P\left(\text{reject }H_{\widehat{j}} \text{ at interim 1}\right)= \P\left( \exists \textbf{C}\supset \widehat{\textbf{I}}_1: \, T_{n,1}^{\widehat{j}}(\id) = T_{n,1}^{(\textbf{C})}(\id) > \widehat{c}_{n,1}^{(\textbf{C})}(1-\alpha) \right) $$
Then, because $\widehat{j}$ is the first hypothesis of $\textbf{I}$ rejected, we have $\widehat{\textbf{I}}_1 \subset \textbf{C}$, hence for any $\sigma \in S_n$, $ T_{n,1}^{(\textbf{C})}(\sigma)\ge T_{n,1}^{(\widehat{\textbf{I}}_1)}(\sigma)$ and the quantiles verify $ \widehat{c}_{n,1}^{(\textbf{C})}(1-\alpha) \ge \widehat{c}_{n,1}^{(\widehat{\textbf{I}}_1)}(1-\alpha)$,
...
\P\left(\text{reject }H_{\widehat{j}} \text{ at interim 1}\right)\le \P\left( T_{n,1}^{(\widehat{\textbf{I}}_1)}(\id) > \widehat{c}_{n,1}^{(\widehat{\textbf{I}}_1)}(1-\alpha) \right) \le  \alpha(1/K) 
\end{equation}
the last inequality is by definition of $\widehat{c}_{n,1}^{(\widehat{\textbf{I}}_1)}(1-\alpha)$
...
$$\forall m < k, \, \exists \textbf{C} \supset \widehat{\textbf{I}}_m  : T_{n,m}^{(\textbf{C})}(\id) \le \widehat{c}_{n,m}^{(\textbf{C})}(1-\alpha).$$
and by definition of $\widehat{c}_{n,k}^{(\widehat{\textbf{I}}_k)}(1-\alpha)$, we have
$$\P\left( T_{n,k}^{(\widehat{\textbf{I}}_k)}(\id)> \widehat{c}_{n,k}^{(\widehat{\textbf{I}}_k)}(1-\alpha)  \Big|\forall m < k, \, \exists \textbf{C} \supset \widehat{\textbf{I}}_m  : T_{n,m}^{(\textbf{C})}(\id) \le \widehat{c}_{n,m}^{(\textbf{C})}(1-\alpha)\right) = \alpha\left(\frac{k}{K}\right)-\alpha\left(\frac{k-1}{K}\right) $$
and similarly to interim 1, we have 
$$T_{n,k}^{(\textbf{C})}(\id)> \widehat{c}_{n,k}^{(\textbf{C})}(1-\alpha)\, \Leftrightarrow \,
 T_{n,k}^{(\widehat{\textbf{I}}_k)}(\id) > \widehat{c}_{n,k}^{(\widehat{\textbf{I}}_k)}(1-\alpha)  $$
 \begin{multline*}
\P\left(\text{reject }H_{\widehat{j}} \text{ at interim k} \Big| \text{did not reject }H_{\widehat{j}} \text{ before interim k}\right)\\
= \P\left( \exists \textbf{C} \supset \widehat{\textbf{I}}_k :\quad  T_{n,k}^{(\textbf{C})}(\id)> \widehat{c}_{n,k}^{(\textbf{C})}(1-\alpha)  \Big|  \forall m < k, \, \exists \textbf{C} \supset \widehat{\textbf{I}}_m  : T_{n,m}^{(\textbf{C})}(\id) \le \widehat{c}_{n,m}^{(\textbf{C})}(1-\alpha)\right)\\
= \P\left(T_{n,k}^{(\widehat{\textbf{I}}_k)}(\id) > \widehat{c}_{n,k}^{(\widehat{\textbf{I}}_k)}(1-\alpha)  \Big|  \forall m < k, \, \exists \textbf{C} \supset \widehat{\textbf{I}}_m  : T_{n,m}^{(\textbf{C})}(\id) \le \widehat{c}_{n,m}^{(\textbf{C})}(1-\alpha) \right)
