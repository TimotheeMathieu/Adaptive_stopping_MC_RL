...
\overline{B}_{N,k}^{(\textbf{C})} = \sup\left\{ b>0 : \frac{1}{m_k}\sum_{\sigma \in \widehat{S}_k} \1\{\underline{T}_{N,k}^{(\textbf{C})}(\sigma_1^k) \leq b\} \le  \underline{q}_k \right\} .
\end{equation}
...
\le   \sum_{k=1}^K  \P_{\sigma_1^k}\left(\overline{T}_{N,k}^{(\textbf{J})}(\id) > \overline{B}_{N,k}^{(\textbf{J})}, \mathrm{NR}_k(\id),\arg\max_{j \in \textbf{J}}T_{N,k}^{(j)}(\id) \in \widehat{\textbf{I}}_k, \forall \textbf{C}_k \supset \textbf{J}'\supsetneq\textbf{J} \quad  \arg\max_{j \in \textbf{J}'}T_{N,k}^{(j)}(\id) \notin \textbf{I} \right)
\end{multline*}
