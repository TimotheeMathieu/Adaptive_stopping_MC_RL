...
Define $mnK$ different seeds $(s_{i,j})_{i\le L,j\le N}$.\\
...
\subsection{Basic results on non-sequential permutation tests}
...
% one of the basic property of permutation tests is that when the null hypothesis is true, then all the permutation are as likely to give a certain value and hence the law given the data is the uniform distributions on all the permutations. The precise theorem for this result is a bit technical due to the difficulty that in a lot of cases, the probability for the data to have any given value is $0$ and we must be a bit careful with the formulation. More precisely, we have this theorem (from \cite[Theorem 17.2.2]{lehmann2005testing})
% \begin{Theorem}\label{th:uniform_permu}
% Let $Z_1^m$ have distribution $P$, and the problem is to test $P \in \mathcal{P}_0$. Suppose that the randomization hypothesis holds that $Z =^d \sigma Z$ for any permutation $\sigma \in S_m$ whenever $P \in \mathcal{P}_0$. Denote $G^z=\{\sigma z, \sigma \in S_m\} $, the orbit of $z \in \R^m$, and let $\mathcal{G}$ be the $\sigma$-field generated by the partition of the sample space described by $G^z$ as $z$ varies. Then for any test statistic $T = T(Z)$, any $P \in \mathcal{P}_0$ and any Borel subset $B\subset \R$, 
% $$\P\left(T(Z)\in B | Z \in \mathcal{G} \right)= \frac{1}{m!}\sum_{\sigma \in S_m} \1\{T(\sigma Z) \in B \} $$ 
% with probability $1$ under $P$.
% \end{Theorem}
% In Theorem~\ref{th:uniform_permu}, one must be careful that in general the event $\{Z \in G^z}\}$ can have probability $0$ for any value of $z$ (in particular this is true if $Z$ has a continuous distribution), hence we have to construct the $\sigma$-field $\mathcal{G}$ to state our theorem properly. A consequence of this theorem is that
...
\P_\sigma\left(\text{E is true} \right)= \sum_{k=1}^K \P_\sigma\left(\text{reject }H_{\widehat{j}} \text{ at interim k} \Big| \text{did not reject }H_{\widehat{j}} \text{ before interim k} \right) 
\end{equation}
...
$$\sum_{\sigma \in S_n} \1\{T_{n}^{(\textbf{C})}(\sigma) \ge  \widehat{b}_{n}^{(\textbf{C})}(1-\alpha)\} = \lfloor \alpha n! \rfloor .$$
