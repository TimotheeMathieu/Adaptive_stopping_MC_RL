...
\begin{description}
\item[Multiprocessing] The algorithm should be able to manage batch of datas.
...
\paragraph{Definition of the boundaries}
With these permutations, we $\overline{B}_{N,k}^{(\textbf{C})}$ and $\underline{B}_{N,k}^{(\textbf{C})}$ such that
\begin{equation}\label{eq:boundary_1}
\overline{B}_{N,k}^{(\textbf{C})} = \inf \left\{b >0 :\frac{1}{m_k}\sum_{\sigma \in \widehat{S}_k} \1\{\overline{T}_{N,k}^{(\textbf{C})}(\sigma_1^k) \ge  b\} \le  \overline{q}_k\right\} 
\end{equation}
and
\begin{equation}\label{eq:boundary_2}
\underline{B}_{N,k}^{(\textbf{C})} = \sup\left\{ b>0 : \frac{1}{m_k}\sum_{\sigma \in \widehat{S}_k} \1\{\underline{T}_{N,k}^{(\textbf{C})}(\sigma_1^k) \leq b\} \le  \underline{q}_k \right\} .
\end{equation}
where $\sum_{j=1}^k\underline{q}_j \le \frac{k\alpha}{K}$ and  $\sum_{j=1}^k\overline{q}_j \le \frac{k\beta}{K}$ and where $\widehat{S}_k$ is the subset of $\mathcal{S}_k$ such that it would not have accepted or rejected before: for each $\sigma_1^k \in \widehat{S}_k$, we have the following property
$$\forall m < k,\quad  \overline{T}_{N,m}^{(\textbf{C})}(\sigma_1^k) \le   \overline{B}_{N,m}^{(\textbf{C})} \text{ and }\underline{T}_{N,m}^{(\textbf{C})}(\sigma_1^k) \ge   \underline{B}_{N,m}^{(\textbf{C})}.$$
\todoT{Explain more the $q_k$}
\begin{algorithm}[h!]
\SetAlgoLined
\SetKwInput{KwParameter}{Parameters}
...
\While{True}{
Compute the boundaries $\overline{B}_{N,k}^{(\textbf{C})}$ and $\underline{B}_{N,k}^{(\textbf{C})}$ from~\eqref{eq:boundary_1} and~\eqref{eq:boundary_2} .\\
\uIf{$\overline{T}_{N,k}^{(\textbf{C})}(\mathrm{id}) > \overline{B}_{N,k}^{(\textbf{C})}$}{
Reject $H_{j_{\max}}$ where $j_{\max} =  \arg\max\left(T_{N,k}^{(j)}(\id),\quad j \in \textbf{C}\right)$.\\
Update $\textbf{C}=\textbf{C} \setminus \{j_{\max }\}$
}
\uElseIf{$\underline{T}_{n,k}^{(\textbf{C})}(\mathrm{id}) < \underline{B}_{N,k}^{(\textbf{C})}$}{
...
\todoT{Explain $P_j=P_k$, how this is limiting and we would prefer to use $\mu_j=\mu_k$. Also show where this stands in the litterature.}
...
To start with, let us demonstrate the performance of our algorithm on several toy examples.
 The first one compares one gaussian distribution $\mathcal{N}(0, 0.1)$ with the mixture of two gaussians $\left(\mathcal{N}(-\Delta/2 , 0.1), \mathcal{N}(\Delta/2, 0.1) \right)$. 
 The second one compares gaussian $\mathcal{N}(0, 0.1)$ with the mixture of gaussians $\left(\mathcal{N}(0 , 0.1), \mathcal{N}(\Delta, 0.1) \right)$. Those two comparisons are presented in Figure~\ref{fig.exp1-2}.
 The last one compares multiple different distributions such as $\mathcal{N}(0, 0.1)$, $\mathcal{N}(0, 0.1)$, mixture $\left(\mathcal{N}(-1 , 0.1), \mathcal{N}(1, 0.1) \right)$, mixture $\left(\mathcal{N}(-1 , 0.1), \mathcal{N}(1, 0.1) \right)$, mixture $\left(\mathcal{N}(-0.2 , 0.1), \mathcal{N}(0.2, 0.1) \right)$, Student's $t$ distribution with $\nu = 3$ and a mean at $0$, mixture $\left(\mathcal{N}(0 , 0.1), \mathcal{N}(8, 0.1) \right)$, mixture $\left(\mathcal{N}(0 , 0.1), \mathcal{N}(8, 0.1) \right)$, mixture of Student's $t$ distributions with $\nu = 3$ and means at $(0, 8)$ and one Student's $t$ distribution with $\nu = 3$ and a mean at $8$ (in Figure~\ref{fig.exp3} those distributions are denoted as N, *N, MG1, *MG1, MG2, tS1, MG3, *MG3, MtS and tS2 respectively). For all threes use cases, we execute Adaptive Stopping algorithm with $K= 5$, $N = 5$ and $\alpha = 0.05$. We also limit the maximum number of permutations to $B = 10 \ 000$.
 ...
 %Moreover, the larger $\Delta$ is then the more stable the test.
 \todo[inline]{AS: maybe cite some lemma here to give intuition behind this empirical result.}
...
Denote 
%$$\mathrm{NR}_k(\sigma_1^k) = \left\{\forall m < k,\quad  \overline{T}_{N,m}^{(\textbf{I})}(\sigma_1^m) \le   \overline{B}_{N,m}^{(\textbf{I})}\right\}.$$
...
&= \sum_{k=1}^K \E\left[\frac{1}{m_k}\sum_{{\sigma_1^k} \in \widehat{S}_k}\1\left\{ \overline{T}_{N,k}^{(\widehat{\textbf{I}}_k)}({\sigma_1^k}) > \overline{B}_{N,k}^{(\widehat{\textbf{I}}_k)}\right\} \right]\\
&\le \sum_{k=1}^K \overline{q}_k \le \alpha
\end{align*}
\todoT{Check the above. In particular, the equality with $\widehat{S}_k$. Maybe define $NR$ for the $T^{(C_k)}\le B_{N,l}$ for every $l \le k$}
...
\todoT{Finish changing notations. And maybe use $\widehat{\textbf{C}}_k$}
...
\le   \sum_{k=1}^K  \P_{\sigma_1^k}\left(\overline{T}_{N,k}^{(\textbf{J})}(\id) > \overline{B}_{N,k}^{(\textbf{J})}, \mathrm{NR}_k(\id,\textbf{C}_k), \mathrm{NA}_k(\id, \textbf{C}_k),\arg\max_{j \in \textbf{J}}T_{N,k}^{(j)}(\id) \in \widehat{\textbf{I}}_k, \forall \textbf{C}_k \supset \textbf{J}'\supsetneq\textbf{J} \quad  \arg\max_{j \in \textbf{J}'}T_{N,k}^{(j)}(\id) \notin \textbf{I} \right)
\end{multline*}
$\mathrm{NR}_k(\id)$ means that we look at the first interim that rejects, and $\mathrm{NA}_k(\id, \textbf{C}_k)$ means that we consider only sample that would ot have accepted any comparison from $\textbf{C}_k$.  $\arg\max_{j \in \textbf{J}}T_{N,k}^{(j)}(\id) \in \widehat{\textbf{I}}_k$ means that we reject a true hypothesis, and $\forall \textbf{C}_k \supset \textbf{J}'\supsetneq \textbf{J} \quad  \arg\max_{j \in \textbf{J}'}T_{N,k}^{(j)}(\id) \notin \textbf{I} $ means that $\textbf{J}$ corresponds to the set for the first rejection of a true hypothesis (there can be several rejection of false hypotheses or acceptation of hypotheses before but not rejection of a true hypothesis). 
We denote 
$$\mathrm{FWE}_k =  \P\left(\exists \textbf{J}:\quad  T_{N,k}^{(\textbf{J}^+)}(\id) > \widehat{b}_{N,k}^{(\textbf{J}^+)}, \mathrm{NR}_k(\id),\mathrm{NA}_k(\id, \textbf{C}_k), \arg\max_{j \in \textbf{J}}T_{N,k}^{(j)}(\id) \in \widehat{\textbf{I}}_k, \forall \textbf{C}_k \supset \textbf{J}'\supsetneq\textbf{J} \quad  \arg\max_{j \in \textbf{J}'}T_{N,k}^{(j)}(\id) \notin \textbf{I} \right),$$ 
...
\todoT{Explain the corrolary that we do $\mu_i  = \mu_j$ hypotheses instead of $P_i=P_j$}
