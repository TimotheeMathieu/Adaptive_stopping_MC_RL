...
\begin{equation*}
\P\left( \left|\sum_{j=1}^k W_j\right| > \frac{b_l}{\tau(P,Q)}, \quad \forall j<k,\, \left|\sum_{i=1}^j W_i\right| \le \frac{b_j}{\tau(P,Q)} \right)=\alpha\left(\frac{k}{K} \right) - \alpha\left( \frac{k-1}{K}\right).
\end{equation*}
...
\begin{itemize}
 \item Asymptotically, the test $H_0:\mu_X = \mu_Y$ vs $H_1:\mu_X \neq \mu_Y$ has level $\alpha$ because under $H_0$, $\tau(P,Q)^2=\sigma_P^2+\sigma_Q^2$ and $T_n(\mathrm{id} E_1^{2n})=\frac{1}{\sqrt{n}}\left(\sum_{i=1}^n E_i - \sum_{i=n+1}^{2n} E_i\right)$ converges to $\mathcal{N}(0,\sigma_X^2+\sigma_Y^2)$. Hence the boundary correspond to the appropriate law.
 \item Under $H_1:\mu_P \neq \mu_Q$, we have an expression of the asymptotic boundary. On the other hand, we also have that
$$T_n(\mathrm{id}E_1^{2n}) =\frac{1}{\sqrt{n}}\left(\sum_{i=1}^n E_i - \sum_{i=n+1}^{2n} E_i\right) = \frac{1}{\sqrt{n}}\left(\sum_{i=1}^n (E_i-\mu_X) - \sum_{i=n+1}^{2n} (E_i-\mu_Y)   + n (\mu_X-\mu_Y)\right)$$
Hence, the test statistics is equivalent to $\sqrt{n}(\mu_X-\mu_Y)$ asymptotically. This gives us a way to choose $n$ and $K$ to have a given power. This also show that the test is consistent in the sense that the power converges to $1$ as $n$ goes to infinity.
\end{itemize}
\todoT{Rk: an asymptotic expansion would be good for the power computation}
