...
Then, the test resulting from Algorithm~\ref{algo:main} has a weak control on the Family-wise error for the multiple test, i.e. if we suppose that all the hypotheses $H_i, i\in \{1,\dots,J\}$ are true, then 
$$ \P\left(\exists 1\le j\le J:\quad  \text{reject }H_j\right)\le\alpha.$$
\end{Theorem}
...
We have that for $k \le K$, for any ${\sigma_1^k}$ concatenation of $k$ permutations, for any $C\subset \I$,
$$(\overline{T}_{N,l}^{(C)}(\id),\overline{B}_{N,l}^{(C)})_{l\le k}=(\overline{T}_{N,l}^{(C)}(\sigma_1^l), \overline{B}_{N,l}^{(C)})_{l\le k}$$
\end{Lemma}
Using Lemma~\ref{lem:invariance}, because $C_k(\id)$ depends only on $(\overline{T}_{N,l}^{(C)}(\id),\overline{B}_{N,l}^{(C)})_{l\le k}$, we have for any ${\sigma_1^k}$
$$\P\left(\overline{T}_{N,k}^{(C)}(\id) > \overline{B}_{N,k}^{(C)}, C_k(\id) = C\right) = \P\left(\overline{T}_{N,k}^{(C)}(\sigma_1^k) > \overline{B}_{N,k}^{(C)}, C_k(\sigma_1^k) = C\right)$$

Hence, from Lemma~\ref{lem:multiple_test_FWE}, and Lemma~\ref{lem:invariance},
\begin{align*}
\mathrm{FWE} &\le \sum_{k=1}^K\sum_{C \subset \I}\frac{1}{m_k}\sum_{\sigma_1^k \in \mathcal{S}_k} \P\left(\overline{T}_{N,k}^{(C)}(\sigma_1^k) > \overline{B}_{N,k}^{(C)}, C_k(\sigma_1^k)=C\right)\\
&=\sum_{k=1}^K\sum_{C \subset \I}\E\left[\frac{1}{m_k}\sum_{\sigma_1^k \in \mathcal{S}_k} \1\left\{\overline{T}_{N,k}^{(C)}(\sigma_1^k) > \overline{B}_{N,k}^{(C)}, C_k(\sigma_1^k)=C\right\}\right]\\
