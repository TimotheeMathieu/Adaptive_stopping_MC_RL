...
Compute $\lambda = \alpha \left(\frac{k}{K}\right)- \alpha \left( \frac{k-1}{K}\right)$ the level of the test.
Compute $\widehat{c}_{n,k}^{(\textbf{C})}(1-\alpha)$ the quantile of order $1-\lambda$ of the permutation law of $T_{n,k}^{(\textbf{C})}(\sigma)$:
$$\sum_{\sigma \in S_n} \1\{T_{n,k}^{(\textbf{C})}(\sigma) \ge  \widehat{c}_{n,k}^{(\textbf{C})}(1-\alpha)\} = \lfloor \lambda n! \rfloor .$$
\If{$T_{n,k}^{(\textbf{C})}(\mathrm{id}) \le \widehat{c}_{n,k}^{(\textbf{C})}(1-\alpha)$}{
...
\todoT{Goal: Show 1) FWE is controlled 2) early accept is also OK for some definition of power 3) asymptotically (maybe for 2 agents only) we control more than $P=Q$ vs $P\neq Q$.} 
...
\begin{Theorem}
The test resulting from Algorithm~\ref{algo:main} has a strong control on the FWE: we have $\mathrm{FWE}\le\alpha$.
\end{Theorem}
\begin{proof}
...
$$\P\left(\text{reject }H_{\widehat{j}} \text{ at interim 1}\right)= \P\left(\exists \textbf{C}: \, T_{n,1}^{\widehat{j}}(\id) = T_{n,1}^{(\textbf{C})}(\id) > \widehat{c}_{n,1}^{(\textbf{C})}(1-\alpha) \right) $$
Then, because $\widehat{j}$ is the first hypothesis of $I$ rejected, we have $\textbf{I} \subset \textbf{C}$, hence for any $\sigma \in S_n$, $ T_{n,1}^{(\textbf{C})}(\sigma)\ge T_{n,1}^{(\textbf{I})}(\sigma)$ and the quantiles verify $ \widehat{c}_{n,1}^{(\textbf{C})}(1-\alpha) \ge \widehat{c}_{n,1}^{(\textbf{I})}(1-\alpha)$,
 and moreover again because $\widehat{j} \in \textbf{I}$ we have
$$T_{n,1}^{\widehat{j}}(\id)=\max(T_{n,1}^{j}(\id), j \in  \textbf{C}) = \max(T_{n,1}^{j}(\id), j \in  \textbf{I}) = T_{n,1}^{(\textbf{I})}(\id).$$
Hence,
\begin{equation}\label{eq:interim1_multi}
\P\left(\text{reject }H_{\widehat{j}} \text{ at interim 1}\right)\le \P\left( T_{n,1}^{(\textbf{I})}(\id) > \widehat{c}_{n,1}^{(\textbf{I})}(1-\alpha) \right) \le  \alpha(1/K) 
\end{equation}
the last inequality is by definition of $\widehat{c}_{n,1}^{(\textbf{I})}(1-\alpha)$

\paragraph{Interim $1<k\le  K$:}

At interim $k$, if we have that none of the hypotheses in $\textbf{I}$ have been rejected, then we are reasoning conditionally on not having rejected before, i.e. we have 
$$ \forall m < k, \, \exists \textbf{C}_j \supset \textbf{I}  : T_{n,m}^{(\textbf{C}_j)}(\id) \le \widehat{c}_{n,m}^{(\textbf{C}_j)}(1-\alpha).$$
and by definition of $\widehat{c}_{n,k}^{(\textbf{I})}(1-\alpha)$, we have
$$\P\left( T_{n,k}^{(\textbf{I})}(\id)> \widehat{c}_{n,k}^{(\textbf{I})}(1-\alpha)  \Big|  \forall m < k, \, \exists \textbf{C}_m \supset \textbf{I}  : T_{n,m}^{(\textbf{C}_m)}(\id) \le \widehat{c}_{n,m}^{(\textbf{C}_m)}(1-\alpha)\right) = \alpha\left(\frac{k}{K}\right)-\alpha\left(\frac{k-1}{K}\right) $$
and similarly to interim 1, we have 
$$T_{n,k}^{(\textbf{C}_{\widehat{j}})}(\id)> \widehat{c}_{n,k}^{(\textbf{C}_{\widehat{j}})}(1-\alpha)\, \Leftrightarrow \,
 T_{n,k}^{(\textbf{I})}(\id) > \widehat{c}_{n,k}^{(\textbf{I})}(1-\alpha)  $$
Hence, 
 \begin{multline*}
\P\left(\text{reject }H_{\widehat{j}} \text{ at interim k} \Big| \text{did not reject }H_{\widehat{j}} \text{ before interim k}\right)\\
= \P\left( T_{n,k}^{(\textbf{C}_{\widehat{j}})}(\id)> \widehat{c}_{n,k}^{(\textbf{C}_{\widehat{j}})}(1-\alpha)  \Big|  \forall m < k, \, \exists \textbf{C}_m \supset \textbf{I}  : T_{n,m}^{(\textbf{C}_m)}(\id) \le \widehat{c}_{n,m}^{(\textbf{C}_m)}(1-\alpha)\right)\\
= \P\left(T_{n,k}^{(\textbf{I})}(\id) > \widehat{c}_{n,k}^{(\textbf{I})}(1-\alpha)  \Big|  \forall m < k, \, \exists \textbf{C}_m \supset \textbf{I}  : T_{n,m}^{(\textbf{C}_m)}(\id) \le \widehat{c}_{n,m}^{(\textbf{C}_m)}(1-\alpha) \right)
