...
and
$$\mathrm{NA}_k(\sigma, \textbf{J}) = \left\{\forall m < k,\quad  T_{N,m}^{(\textbf{J}^-)}(\sigma_1\cdot\ldots\cdot\sigma_m) \ge   \widehat{b}_{N,m}^{(\textbf{J}^-)}\right\}$$
$\mathrm{NR}_k(\sigma)$ is the event on which the statistics corresponding to true hypotheses, computed after permutation by $\sigma=\sigma_1\cdot \ldots \cdot \sigma_k$, are not rejected and $\mathrm{NA}_k(\sigma, \textbf{J})$ is the event on which none of the comparison from $\textbf{J}$ have been rejected after permutation by $\sigma$.
...
\P_\sigma\left(\text{E is true} \right)= \sum_{k=1}^K \P\left( \exists j \in \textbf{I}: \quad H_j\text{ is rejected at interim $k$ }|\text{there were no rejection before interim $k$}\right)\\
\le   \sum_{k=1}^K  \P\left(\exists \textbf{J}:\quad  T_{N,k}^{(\textbf{J}^+)}(\id) > \widehat{b}_{N,k}^{(\textbf{J}^+)}, \mathrm{NR}_k(\id), \mathrm{NA}_k(\id, \textbf{J}), \arg\max_{j \in \textbf{J}}T_{N,k}^{(j)}(\id) \in \textbf{I}, \forall \textbf{C}_k \supset \textbf{J}'\supsetneq\textbf{J} \quad  \arg\max_{j \in \textbf{J}'}T_{N,k}^{(j)}(\id) \notin \textbf{I} \right)
\end{multline*}
...
We denote 
$$\mathrm{FWE}_k =  \P\left(\exists \textbf{J}:\quad  T_{N,k}^{(\textbf{J}^+)}(\id) > \widehat{b}_{N,k}^{(\textbf{J}^+)}, \mathrm{NR}_k(\id), \mathrm{NA}_k(\id, \textbf{J}), \arg\max_{j \in \textbf{J}}T_{N,k}^{(j)}(\id) \in \textbf{I}, \forall \textbf{C}_k \supset \textbf{J}'\supsetneq\textbf{J} \quad  \arg\max_{j \in \textbf{J}'}T_{N,k}^{(j)}(\id) \notin \textbf{I} \right),$$ 
the error of interim $k$ and by convention we take $f^+(-1/K)=0$.

Let $\textbf{J}\cap \textbf{I} = \textbf{I}_k$.
...
$$T_{N,k}^{(\textbf{J}^+)}(\id)= \max\{T_{N,k}^{(j)}(\id), j \in \textbf{J}\} = \max\{T_{N,1}^{(j)}(\id), j \in \textbf{I}\cap \textbf{J}\} = T_{N,k}^{(\textbf{I}_k^+)}(\id)$$
Moreover, having $\textbf{J} \supset \textbf{I}_k$, we have $\widehat{b}_{N,k}^{(\textbf{J}^+)} \ge \widehat{b}_{N,k}^{(\textbf{I}_k^+)}$. And finally, we have 
\begin{align*}
\mathrm{FWE}_k\le \P\left(\exists \textbf{J} : \quad T_{N,k}^{(\textbf{I}_k^+)}(\id) > \widehat{b}_{N,k}^{(\textbf{I}_k^+)}, \mathrm{NR}_k(\id), \mathrm{NA}_k(\id, \mathrm{J}), \textbf{J}\cap \textbf{I} = \textbf{I}_k \right)
\end{align*}
Then, having that $\mathrm{NA}_k(\id, \mathrm{J}) \Rightarrow \mathrm{NA}_k(\id, \mathrm{I}_k)$, we have that 
\begin{align*}
\mathrm{FWE}_k\le \P\left(\exists \textbf{I}_k \subset \textbf{I} : \quad T_{N,k}^{(\textbf{I}_k^+)}(\id) > \widehat{b}_{N,k}^{(\textbf{I}_k^+)}, \mathrm{NR}_k(\id), \mathrm{NA}_k(\id, \mathrm{I}_k) \right)
\end{align*}
...
\begin{align*}
\mathrm{FWE}_k &\le \frac{1}{((2N)!)^k}\sum_{\sigma_1,\dots,\sigma_k \in \S_{2N}}\P\left(\exists \textbf{J}:\quad  T_{N,k}^{(\textbf{I}_k^+)}(\sigma) > \widehat{b}_{N,k}^{(\textbf{I}_k^+)}, \mathrm{NR}_k(\sigma),\mathrm{NA}_k(\sigma, \textbf{I}_k)) \right)\\
