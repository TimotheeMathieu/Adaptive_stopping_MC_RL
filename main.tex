...
\cite{Sigaud}'s work share the same reproducible science objective as our's. \cite{Sigaud}'s uses non-adaptive tests to compare RL algortihms (\cite{Sigaud}-Sec. 3.2) whearas AdaStop (\ref{sec:main_algo}) is adaptive and designed specifically for rl aglorithms comparisons. We follow \cite{Sigaud}'s experimental protocole and compare AdaStop and non-adaptive approaches empirically in terms of false positive rate and statistical
power as a function of the sample size (number of seeds). In particular, we use \cite{Sigaud}'s \href{https://github.com/flowersteam/rl_stats/tree/master/data}{data} of SAC \cite{SAC} and TD3 \cite{TD3} evaluations on HalfCheetah to obtain two multimodal distributions (fig. \ref{fig:sactd3}). In the fashion of \cite{Sigaud}-table 15, we compute the empirical statistical power of AdaStop as a function of the number of seeds of the RL algortihms (table. \ref{tab:stat_power_sactd3}). To compute the empirical statistical power for a given number of seeds, we assume that the evaluations distributions of SAC and TD3 are different and we count how many times AdaStop decides that one agent is better than the other (number of true positives). And because the test is adaptive, we also report the effective number of seeds necessary to make a decision with $0.95$ confidence level. For each number of seeds, we launched AdaStop $1000$ times and average statistical power and effective number of seeds used. For example, when comparing SAC and TD3 performances on HalfCheetah using AdaStop with $n=4$ and $k=5$, the maximumun number of seeds that could be used is $n \times k = 20$ in the case of no early stopping. However we observe on table. \ref{tab:stat_power_sactd3} that when $n=4$ and $k=5$, AdaStop can make a decision with a power of 0.82 using only 12 seeds. In \cite{Sigaud}-table 15, the minimum number of seeds required to obtain a statistical power of 0.8 when comparing SAC and TD3 is 15 when using a t-test, a Welch test, or a bootstrapping test.
\begin{figure}
	\begin{centering}
...
\begin{table}
	...
	\begin{tabular}{rlllll}
		...
		\hline
	\end{tabular}
\caption{Empirical statistical power of AdaStop as a function of number of seeds when comparing SAC and TD3 on HalfCheetah.\label{tab:stat_power_sactd3}}
\end{table}
