...
class AgentComparator:
    ...
    def explore_graph(self, k, Rs, boundary):
        ...
        for j in range(k + 1):
            ...
            for f in range(2 * self.n):  # Stage f of the network
                ...
                cs = csnew
            if j != k:
                records, rec_nodes = self.postprocess_records(
                    records, boundary, rec_nodes
                )
    ...
    def compare(self, manager1, manager2):
        ...
        for k in range(self.K):
            ...
            cdf_emp = np.cumsum(probas[idx])
            bk = np.max(values[cdf_emp <= 1 - clevel + level_spent])
            boundary.append(bk)

            
            T = np.sum(Rs[-1] * X)
            level_spent += 1 - np.max(cdf_emp[cdf_emp < 1 - clevel + level_spent])
